...
const OrderPage: React.FC = () => {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'card' | 'cod' | 'koko'>('card');
  ...
  useEffect(() => {
    try {
      const stored = JSON.parse(localStorage.getItem('viton_cart') || '[]');
      setCart(stored);
    } catch { setCart([]); }
  }, []);
        ...
        <div style={successWrapStyle}>
          <div style={successBoxStyle}>
            ...
            <div style={successSummaryStyle}>
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', paddingTop: '12px', borderTop: '1px solid #e8e4de' }}>
                <span style={{ ...summaryLabelStyle, color: '#1a1a1a', fontWeight: 500 }}>Total Paid</span>
                <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '22px', color: '#c9a96e' }}>LKR {total.toFixed(2)}</span>
              </div>
            </div>
          ...
          <section style={sectionStyle}>
            ...
            <div style={shippingOptionsStyle}>
              {([
                ...
                <label
                  key={opt.id}
                  style={{ ...shippingOptionStyle, borderColor: shippingMethod === opt.id ? '#c9a96e' : '#e8e4de', background: shippingMethod === opt.id ? '#fdf9f3' : '#fff' }}
                >
                  <input
                    type="radio"
                    name="shipping"
          ...
          <section style={sectionStyle}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginBottom: '20px' }}>
              {([
                ...
                { id: 'koko', label: 'Koko · Buy Now, Pay Later' },
                { id: 'cod',  label: 'Cash on Delivery' },
              ] as const).map(opt => (
                <label
                  key={opt.id}
                  style={{ ...shippingOptionStyle, borderColor: paymentMethod === opt.id ? '#c9a96e' : '#e8e4de', background: paymentMethod === opt.id ? '#fdf9f3' : '#fff' }}
                >
            ...
            {paymentMethod === 'card' && (
              <div style={{ border: '1px solid #e8e4de', padding: '20px', background: '#fafaf8' }}>
                <Field label="Card number" error={errors.cardNumber}>
                  <input style={inputStyle(!!errors.cardNumber)} placeholder="1234 5678 9012 3456" maxLength={19} value={form.cardNumber} onChange={e => setForm(f => ({ ...f, cardNumber: e.target.value.replace(/\D/g,'').replace(/(\d{4})/g,'$1 ').trim() }))} />
                </Field>
                <div style={rowStyle}>
            ...
            {paymentMethod === 'koko' && (
              <div style={{ border: '1px solid #e8e4de', padding: '20px', background: '#fafaf8', fontFamily: "'Montserrat',sans-serif", fontSize: '12px', color: '#6b6560', lineHeight: 1.8 }}>
                Split your payment into 3 interest-free instalments with Koko. You will be redirected to complete your purchase after placing the order.
              </div>
            )}
          ...
          <div style={summaryCardStyle}>
                  ...
                  <div key={`${pid}-${item.selectedSize}`} style={orderItemStyle}>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      <div style={{
                        ...
                        background: `linear-gradient(135deg, ${getCategoryColor(item.product.category)}, #e8ddd0)`,
            ...
            <div style={{ borderTop: '1px solid #e8e4de', paddingTop: '16px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
              ...
              <div style={{ ...totalRowStyle, paddingTop: '12px', borderTop: '1px solid #e8e4de' }}>
                <span style={{ fontFamily: "'Montserrat',sans-serif", fontSize: '11px', fontWeight: 500, letterSpacing: '2px', textTransform: 'uppercase', color: '#1a1a1a' }}>Total</span>
                <div style={{ textAlign: 'right' }}>
                  <span style={{ fontFamily: "'Montserrat',sans-serif", fontSize: '9px', color: '#9a9590', display: 'block', marginBottom: '2px' }}>LKR</span>
                  <span style={{ fontFamily: "'Cormorant Garamond',serif", fontSize: '28px', fontWeight: 300, color: '#1a1a1a' }}>
                    {total.toFixed(2)}
                  </span>
                </div>
              </div>
...
const layoutStyle: React.CSSProperties = { display: 'grid', gridTemplateColumns: '1fr 420px', minHeight: 'calc(100vh - 65px)', maxWidth: '1200px', margin: '0 auto', gap: '0' };
...
const sectionTitleStyle: React.CSSProperties = { fontFamily: "'Cormorant Garamond',serif", fontSize: '22px', fontWeight: 300, color: '#1a1a1a', marginBottom: '18px' };
...
const successTitleStyle: React.CSSProperties = { fontFamily: "'Cormorant Garamond',serif", fontSize: '36px', fontWeight: 300, color: '#1a1a1a', marginBottom: '12px' };
...
const successSummaryStyle: React.CSSProperties = { background: '#fafaf8', border: '1px solid #e8e4de', padding: '20px', marginBottom: '28px', textAlign: 'left' };
const primaryBtnStyle: React.CSSProperties = { padding: '14px 32px', background: '#1a1a1a', color: '#fff', fontFamily: "'Montserrat',sans-serif", fontSize: '10px', fontWeight: 500, letterSpacing: '2.5px', textTransform: 'uppercase', textDecoration: 'none', display: 'inline-block', transition: 'background 0.25s' };
