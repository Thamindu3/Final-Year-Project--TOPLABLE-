...
class VITONDataset(data.Dataset):
    ...
    def get_img_agnostic(self, img, parse, pose_data):
        ...
        # If hip/knee keypoints are missing (zeroed out), estimate from mid_hip (idx 8)
        if length_b < 1.0 and np.any(pose_data[8] > 0):
            half_w = max(length_a * 0.45, 50.0)
            pose_data[9]  = pose_data[8] + np.array([-half_w, 0])
            pose_data[12] = pose_data[8] + np.array([ half_w, 0])
            length_b = np.linalg.norm(pose_data[12] - pose_data[9])
        ...
        # Restrict gray masking to person region only — prevent bleed into background
        bg_mask = (parse_array == 0).astype(np.uint8) * 255
        agnostic.paste(img, None, Image.fromarray(bg_mask, 'L'))
    ...
    def __getitem__(self, index):
        ...
        with open(osp.join(self.data_path, 'openpose-json', pose_name), 'r') as f:
            ...
            # Zero out keypoints whose coordinates fall outside the image boundary
            out_of_bounds = (pose_data[:, 0] < 0) | (pose_data[:, 0] >= self.load_width) | \
                            (pose_data[:, 1] < 0) | (pose_data[:, 1] >= self.load_height)
            pose_data[out_of_bounds] = 0.0
        ...
        # Use pre-computed SegFormer agnostic for uploaded photos (cleaner boundaries)
        precomputed_agnostic = osp.join(self.data_path, 'agnostic-v3.2', img_name)
        if osp.exists(precomputed_agnostic):
            img_agnostic = Image.open(precomputed_agnostic).convert('RGB')
